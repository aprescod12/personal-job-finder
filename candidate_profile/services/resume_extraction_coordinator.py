...
MANUAL_REVIEW_VERSION = "manual-resume-review-v1"


@dataclass(slots=True)
class ResumeExtractionAttempt:
    provider_path: str
    provider_key: str = ""
    provider_label: str = ""
...
def _safe_failure(exc: Exception) -> ResumeExtractionError:
    if isinstance(exc, ResumeExtractionError):
        return exc
    return ResumeExtractionError(
        "The resume extraction provider failed unexpectedly.",
...
def _attempt_extraction(
    ...
    *,
    provider_path: str,
    extractor: BaseResumeExtractor | None,
    clock: Callable[[], float],
) -> tuple[dict[str, object] | None, ResumeExtractionAttempt, BaseResumeExtractor | None]:
    started_at = clock()
    attempt = ResumeExtractionAttempt(provider_path=provider_path)

    try:
        active_extractor = extractor or get_resume_extractor(provider_path)
...
def _manual_review_payload(
    *,
    ...
    return ResumeExtractionResult(
        provider_key="manual_resume_review",
        provider_label="Manual resume review",
...
def _orchestration_payload(
    *,
    ...
    attempts: list[ResumeExtractionAttempt],
    fallback_used: bool,
...
def extract_resume_with_fallback(
    request: ResumeExtractionRequest,
    *,
    primary_extractor: BaseResumeExtractor | None = None,
    fallback_extractor: BaseResumeExtractor | None = None,
    fallback_enabled: bool | None = None,
    clock: Callable[[], float] = perf_counter,
) -> dict[str, object]:
    """Produce a review draft while disclosing provider and fallback behavior."""
    ...
    use_fallback = (
        getattr(settings, "RESUME_FALLBACK_ENABLED", True)
    ...
    if should_try_fallback:
        ...
        if fallback_payload is not None:
            fallback_payload["warnings"] = [
                (
                    "The primary resume extractor was unavailable. This draft was "
                    "produced by the deterministic fallback and requires careful review."
                ),
                (
    ...
    failure_warnings = [
        (
            "No configured resume extractor produced a structured draft. The locally "
            "parsed resume text remains available below for manual review, and nothing "
            "has been applied to the career profile."
        ),
        (
            f"Primary extractor issue ({primary_attempt.error_category}): "
            f"{primary_attempt.error_message}"
    ...
    if len(attempts) > 1:
        ...
        failure_warnings.append("Automatic resume fallback is disabled by configuration.")
    elif same_configured_provider or same_injected_provider:
        failure_warnings.append(
            "Resume fallback was skipped because it matched the failed primary extractor."
        )

    manual_payload = _manual_review_payload(warnings=failure_warnings)
    manual_payload["orchestration"] = _orchestration_payload(
        status="all_extractors_failed",
        primary_path=primary_path,
